...
window.addEventListener("DOMContentLoaded", function (event) {
  ...
  window.addEventListener("keydown", function (event) {
    ...
    if (event.key == "H" || event.key == "h") {
      ...
      noteElement.innerHTML = `Eu Sou a Morte Encarnada (Modo Dificil). Aperte espaço para começar!`;
    ...
    if (event.key == "E" || event.key == "e") {
      ...
      noteElement.innerHTML = `Posso jogar mamãe? (Modo Facil). Aperte espaço para começar!`;
      noteElement.style.opacity = 1;
      resetGame();
      return;
    }
  ...
  function main(timestamp) {
    try {
      ...
      const pressSpaceToStart = "Pressione Espaco para resetar o Jogo.";
      const changeMode = hardMode
        ? "Voltar ao modo Facil? Pressione E."
        : "Preparado para um desafio? Pressione H.";
      const followMe =
        "Nao desista tente Novamente :)" ;
  ...
  function getNextPosition() {
    ...
    switch (snakeDirection) {
      case "right": {
        ...
        if (nextPosition % width == 0) throw Error("TTome cuidado sua cobra bateu no lugar errado!");
        // Ignore the last snake part, it'll move out as the head moves in
        if (snakePositions.slice(1).includes(nextPosition))
          throw Error("A cobra se comeu!");
        return nextPosition;
      }
      case "left": {
        const nextPosition = headPosition - 1;
        if (nextPosition % width == width - 1 || nextPosition < 0)
          throw Error("Tome cuidado sua cobra bateu no lugar errado!");
        // Ignore the last snake part, it'll move out as the head moves in
        if (snakePositions.slice(1).includes(nextPosition))
          throw Error("A cobra se comeu!");
        return nextPosition;
      ...
      case "up": {
        const nextPosition = headPosition - width;
        if (nextPosition < 0) throw Error("Tome cuidado sua cobra bateu no lugar errado!");
        // Ignore the last snake part, it'll move out as the head moves in
        if (snakePositions.slice(1).includes(nextPosition))
          throw Error("A cobra se comeu!");
        return nextPosition;
